...
% \end{theorem}
% For a proof see \cite[Chapter 4.3]{RiemannianGeometry2006}.
...
Note now \( u-t \) is still harmonic, has Dirichlet boundary condition \( u-t=0 \) on \( \Sigma' \), and Neumann boundary condition \( \partial_n (u-t)=0 \) on \( \Sigma \). Hence we can apply \Cref{thm:maximum_principle} and get that \( u \) must be constant on \( E \), which contradicts the assumption that \( t \) is a regular value.
